    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: isMobile ? "16px 14px" : 24 }}>
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "stretch" : "center", marginBottom: 16, gap: 10 }}>
        <div style={{ position: "relative", flex: 1, maxWidth: isMobile ? "100%" : 320 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search contacts..." style={{ width: "100%", padding: "10px 12px 10px 36px", borderRadius: 8, border: "1px solid #1e293b", background: "#1e293b", color: "#e2e8f0", fontSize: 14, fontFamily: "inherit", outline: "none" }} />
        </div>
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ company: "", contactName: "", phone: "", email: "", address: "", notes: "", type: "customer" }); }}
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 16px", borderRadius: 8, border: "none", background: "#6366f1", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", minHeight: 44, flexShrink: 0 }}><Plus size={16} /> Add Contact</button>
      </div>
      ...
      {filtered.map(contact => (
        <div key={contact.id} style={{ background: "#1a2332", borderRadius: 12, border: "1px solid #1e293b", padding: isMobile ? "14px 14px" : "16px 20px", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                ...
                {projectCount(contact) > 0 && <span style={{ fontSize: 10, padding: "3px 8px", borderRadius: 10, background: "#0f1729", color: "#94a3b8", fontWeight: 600 }}>{projectCount(contact)} projects</span>}
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
              <button onClick={() => { setForm(contact); setEditId(contact.id); setShowForm(true); }} style={{ width: 32, height: 32, borderRadius: 6, background: "transparent", border: "1px solid #1e293b", color: "#94a3b8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Edit2 size={14} /></button>
              <button onClick={() => onSave((contacts || []).filter(c => c.id !== contact.id))} style={{ width: 32, height: 32, borderRadius: 6, background: "transparent", border: "1px solid #1e293b", color: "#475569", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={14} /></button>
            </div>
          </div>
        </div>
      ))}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 20 }}>
              <button onClick={() => setShowForm(false)} style={{ flex: isMobile ? 1 : "0 0 auto", padding: "12px 20px", borderRadius: 8, border: "1px solid #1e293b", background: "transparent", color: "#cbd5e1", fontSize: 14, cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>Cancel</button>
              <button onClick={save} style={{ flex: isMobile ? 2 : "0 0 auto", padding: "12px 24px", borderRadius: 8, border: "none", background: "#6366f1", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", opacity: form.company.trim() ? 1 : 0.4 }}>{editId ? "Save" : "Add Contact"}</button>
            </div>
          </div>
        </div>
      )}
