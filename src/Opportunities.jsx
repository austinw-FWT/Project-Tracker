...
import { Plus, X, Edit2, ArrowRight, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: 24 }}>
      ...
      {filtered.sort((a, b) => (b.updatedAt || "").localeCompare(a.updatedAt || "")).map(opp => {
        ...
        return (
          <div key={opp.id} style={{ background: "#1a2332", borderRadius: 12, border: "1px solid #1e293b", padding: "16px 20px", marginBottom: 10 }}>
            ...
            {opp.scopeNotes && <div style={{ marginTop: 8, fontSize: 12, color: "#94a3b8", paddingLeft: 28 }}>{opp.scopeNotes}</div>}
            {opp.projectTypes?.length > 0 && <div style={{ display: "flex", gap: 4, marginTop: 6, paddingLeft: 28 }}>{opp.projectTypes.map(t => <span key={t} style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "#0f1729", color: "#94a3b8" }}>{t}</span>)}</div>}
          </div>
        );
      })}
